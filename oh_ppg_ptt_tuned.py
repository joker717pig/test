...
THRESHOLDS = {
    # PTT 使用 ECG-R 到 PPG 波脚（foot）计算，阈值放宽到更适合 PPG 场景
    'ptt_low': 120,
    'ptt_high': 320,
    'ptt_severe_low': 100,
    'ptt_severe_high': 360,
...
def detect_ppg_peaks(ppg_signal, fs):
    ...
    feet = []
    for peak in peaks:
        search_window = int(0.35 * fs)
        start_idx = max(0, peak - search_window)
        segment = ppg_norm[start_idx:peak]
        if len(segment) < 5:
            continue

        # 先找峰前局部谷值，再在谷值到主峰之间找最大上升斜率点作为 foot
        valley_rel = int(np.argmin(segment))
        valley_idx = start_idx + valley_rel
        rise_segment = ppg_norm[valley_idx:peak]
        if len(rise_segment) >= 3:
            slope = np.diff(rise_segment)
            foot_rel = int(np.argmax(slope))
            foot_idx = valley_idx + foot_rel
        else:
            foot_idx = valley_idx

        if foot_idx < peak:
            feet.append(int(foot_idx))

    return peaks, feet, ppg_norm


def calculate_ptt_ecg_ppg(ecg_signal, ppg_signal, fs):
    """
    使用 ECG R 波到 PPG 波脚（foot）计算 PTT。
    相比直接配 PPG 波峰，foot 更接近真实到达时间，偏差更小。
    """
    r_peaks, _, _ = detect_r_peaks_ecg(ecg_signal, fs)
    ppg_peaks, ppg_feet, _ = detect_ppg_peaks(ppg_signal, fs)

    candidate_points = np.asarray(ppg_feet if len(ppg_feet) >= 3 else ppg_peaks, dtype=int)
    if len(r_peaks) < 3 or len(candidate_points) < 3:
        return None, False

    ptt_values = []
    valid_pairs = []
    min_delay = int(0.12 * fs)
    max_delay = int(0.35 * fs)

    for r_peak in r_peaks[-50:]:
        subsequent = candidate_points[(candidate_points > r_peak + min_delay) & (candidate_points < r_peak + max_delay)]
        if len(subsequent) == 0:
            continue
        target = int(subsequent[0])
        ptt_ms = (target - r_peak) / fs * 1000.0
        if 80 <= ptt_ms <= 400:
            ptt_values.append(ptt_ms)
            valid_pairs.append((int(r_peak), target))

    if len(ptt_values) < 3:
        return None, False

    ptt_values = np.asarray(ptt_values, dtype=float)
    median_ptt = float(np.median(ptt_values))
    mad = float(np.median(np.abs(ptt_values - median_ptt)))
    if mad > 0:
        mask = np.abs(ptt_values - median_ptt) <= max(2.5 * 1.4826 * mad, 25.0)
        ptt_values = ptt_values[mask]
        valid_pairs = [pair for pair, keep in zip(valid_pairs, mask) if keep]

    if len(ptt_values) < 3:
        return None, False

    ptt_mean = float(np.median(ptt_values))
    ptt_std = float(np.std(ptt_values))
    ptt_cv = ptt_std / ptt_mean if ptt_mean > 0 else 0
    return {
        'ptt_mean': ptt_mean,
        'ptt_std': ptt_std,
        'ptt_cv': ptt_cv,
        'ptt_count': int(len(ptt_values)),
        'valid_pairs': valid_pairs,
        'ptt_method': 'ECG-R to PPG-foot',
    }, True
...
def calculate_risk_level(features, thresholds=None):
    ...
    计算风险等级 (0-4)。
    调整点：
    1. 优先使用 ECG-R 到 PPG-foot 的 PTT；
    2. 只有 PTT 明显异常或 PTT 异常 + 其他指标共现时，才提升到 OH 风险；
    3. 参考第二份串口代码的分级逻辑，降低“误报偏高”。
    """
    if not features:
        return 0, '数据不足'
    ...
    has_ptt = bool(features.get('has_ptt', False) and ptt_info)
    ...
    if has_ptt:
        ptt_mean = float(ptt_info.get('ptt_mean', 0))
        ptt_cv = float(ptt_info.get('ptt_cv', 0))
        ptt_count = int(ptt_info.get('ptt_count', 0))

        if ptt_count >= 3:
            if ptt_mean < thresholds['ptt_severe_low'] or ptt_mean > thresholds['ptt_severe_high']:
                severe_ptt = True
                abnormal_ptt = True
                reasons.append(f'PTT明显异常 ({ptt_mean:.1f}ms)')
            elif ptt_mean < thresholds['ptt_low'] or ptt_mean > thresholds['ptt_high']:
                abnormal_ptt = True
                reasons.append(f'PTT偏离正常 ({ptt_mean:.1f}ms)')

            if ptt_cv > thresholds['ptt_cv_high']:
                support_score += 1
                reasons.append(f'PTT波动较大 (CV:{ptt_cv:.2f})')
            elif ptt_cv > thresholds['ptt_cv_mid']:
                reasons.append(f'PTT轻度波动 (CV:{ptt_cv:.2f})')

    hr = 0
    if hrv_features:
        hr = float(hrv_features.get('heart_rate', 0))
        rmssd = float(hrv_features.get('rmssd', 0))
        pnn50 = float(hrv_features.get('pnn50', 0))

        if hr > thresholds['hr_very_high']:
            support_score += 2
            reasons.append(f'心率明显偏高 ({hr:.1f}bpm)')
        elif hr > thresholds['hr_high'] or (0 < hr < thresholds['hr_low']):
            support_score += 1
            reasons.append(f'心率异常 ({hr:.1f}bpm)')

        if 0 < rmssd < thresholds['rmssd_low']:
            support_score += 1
    ...
    if ppg_features:
        amp_cv = float(ppg_features.get('ppg_amp_cv', 0))
        rise_time = float(ppg_features.get('rise_time_mean', 0))
        pulse_rate = float(ppg_features.get('pulse_rate', 0))

        if hr > 0 and pulse_rate > 0 and abs(pulse_rate - hr) > 8:
            support_score += 1
            reasons.append(f'脉率-心率不一致 ({abs(pulse_rate - hr):.1f}bpm)')

        if amp_cv > thresholds['amp_cv_high']:
            support_score += 1
            reasons.append(f'PPG幅值不稳定 (CV:{amp_cv:.1f}%)')

        if rise_time > thresholds['rise_time_high'] or (0 < rise_time < thresholds['rise_time_low']):
            support_score += 1
            reasons.append(f'PPG上升时间异常 ({rise_time:.1f}ms)')

    if 0 < spo2 < thresholds['spo2_low']:
        support_score += 1
        reasons.append(f'血氧偏低 (SpO₂:{spo2:.1f}%)')
    if 0 < pi < thresholds['pi_low']:
        support_score += 1
        reasons.append(f'灌注指数低 (PI:{pi:.2f}%)')

    if not has_ptt:
        return 0, '数据不足，尚未得到稳定PTT'

    # 分级：参考第二份串口代码，但避免仅凭一个边缘异常就拉高风险
    if severe_ptt and support_score >= 1:
        risk_level = 4
    elif severe_ptt or (abnormal_ptt and support_score >= 2):
        risk_level = 3
    elif abnormal_ptt and support_score >= 1:
        risk_level = 2
    else:
        risk_level = 1

    reason_str = '; '.join(dict.fromkeys(reasons)) if reasons else '正常'
...
class RealTimeProcessor(QObject):
    ...
    def process_features(self, latest_data=None):
        ...
        ppg_peaks, ppg_feet, _ = detect_ppg_peaks(ppg_array, self.fs)
        if len(ppg_peaks) > 0:
            offset = len(self.ppg_red_buffer) - len(ppg_array)
            display_points = ppg_feet if len(ppg_feet) >= 3 else ppg_peaks
            self.ppg_peaks = [offset + int(p) for p in display_points]
