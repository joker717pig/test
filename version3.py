...
稳定版修复：
1. 串口线程只负责收数，不直接高频触发界面重绘
2. 界面改为QTimer定时刷新，避免每个样本都重画导致卡死
3. 使用deque代替list+pop(0)，降低长时间运行开销
4. 限制调试窗口行数，避免文本框无限膨胀
5. 自动缩放节流，只在较低频率下更新视图范围
6. 【新增】支持串口输出OH判定结果到STM32等设备
"""
...
# 串口输出配置
OUTPUT_SERIAL_PORT = ""    # 输出串口号，留空则不启用输出
OUTPUT_BAUDRATE = 115200  # 输出波特率
OUTPUT_INTERVAL = 5.0     # OH结果发送间隔（秒）
...
def oh_judgment_model_ecg_ppg(features, thresholds=None):
    ...
    OH判定模型：基于ECG+PPG特征
    返回：(是否OH, 原因描述, 风险评分)
    风险等级：0-数据不足, 1-正常, 2-轻度, 3-中度, 4-重度
    """
    if not features:
        return False, '数据不足', 0
    ...
    if risk_score >= 2 and has_ptt:
        return True, f"中度风险: {'; '.join(reasons)}", risk_score
    return False, f'风险评分:{risk_score}分', risk_score


def calculate_risk_level(is_oh, reason, risk_score, features):
    """
    将OH判定结果转换为风险等级（0-4）
    0: 数据不足
    1: 正常
    ...
    hr_mean = features.get('hrv_features', {}).get('heart_rate', 0)
    
    # 重度：PTT极短或极长，或心率异常
    if ptt_mean < 100 or ptt_mean > 300 or hr_mean > 120 or hr_mean < 40:
        return 4
    # 中度
    elif ptt_mean < 120 or ptt_mean > 280 or hr_mean > 110 or hr_mean < 45:
        return 3
    # 轻度
    else:
        return 2
...
class SerialTextReceiver(QThread):
    """串口接收线程：从设备接收数据"""
    ...
    def connect(self):
        try:
            self.serial = serial.Serial(port=self.port, baudrate=self.baudrate, timeout=TIMEOUT)
            self.running = True
            self.connection_status.emit(True)
            self.debug_message.emit(f'输入串口已连接: {self.port} @ {self.baudrate}')
            return True
        except Exception as e:
            self.debug_message.emit(f'输入串口连接失败: {e}')
    ...
    def disconnect(self):
        ...
        if self.serial and self.serial.is_open:
            try:
                ...
                pass
        self.connection_status.emit(False)
        self.debug_message.emit('输入串口已断开')
            ...
            try:
                if self.serial and self.serial.is_open:
                    ...
                    if data is not None:
                        ...
                        now = time.time()
                        if now - self.last_bad_log > 2.0:
                            self.debug_message.emit(f'收到未识别行: {line[:120]}')
                            self.last_bad_log = now
                else:
                    time.sleep(0.02)
            except Exception as e:
                self.debug_message.emit(f'接收错误: {e}')
                time.sleep(0.1)


class SerialOutputSender(QThread):
    """串口发送线程：向设备发送OH判定结果"""
    connection_status = pyqtSignal(bool)
    debug_message = pyqtSignal(str)
    send_success = pyqtSignal(int, str)  # 发送成功信号：风险等级, 原因

    def __init__(self, port=OUTPUT_SERIAL_PORT, baudrate=OUTPUT_BAUDRATE):
        super().__init__()
        self.port = port
        self.baudrate = baudrate
        self.serial = None
    ...
    def connect(self):
        """连接输出串口"""
        if not self.port:
            self.debug_message.emit('输出串口未配置，跳过连接')
            return False
        try:
            ...
            self.enabled = True
            self.connection_status.emit(True)
            self.debug_message.emit(f'输出串口已连接: {self.port} @ {self.baudrate}')
            return True
        except Exception as e:
            self.debug_message.emit(f'输出串口连接失败: {e}')
            self.connection_status.emit(False)
            self.enabled = False
            return False
    ...
    def disconnect(self):
        ...
        if self.serial and self.serial.is_open:
            try:
                ...
                pass
        self.connection_status.emit(False)
        self.debug_message.emit('输出串口已断开')

    def update_risk_level(self, risk_level, reason):
        """更新风险等级（线程安全）"""
    ...
    def run(self):
        ...
        while self.running:
            try:
                if self.serial and self.serial.is_open and self.enabled:
                    current_time = time.time()
                    
                    # 检查是否有新数据且到了发送时间
                    QMutexLocker(self.lock)
                    if self.new_data_available and (current_time - last_send_time >= OUTPUT_INTERVAL):
                        # 发送格式: "RISK:X\n" 其中X为0-4
                        message = f"RISK:{self.current_risk_level}\n"
                        
                        try:
                            self.serial.write(message.encode('utf-8'))
                            self.send_success.emit(self.current_risk_level, self.current_reason)
                            self.debug_message.emit(f'已发送风险等级: {self.current_risk_level} - {self.current_reason}')
                            last_send_time = current_time
                        except Exception as e:
                            self.debug_message.emit(f'串口发送失败: {e}')
                        
                        self.new_data_available = False
                
                time.sleep(0.1)
            except Exception as e:
                self.debug_message.emit(f'发送线程异常: {e}')
                time.sleep(0.5)
...
class RealTimeProcessor(QObject):
    ...
    def process_features(self, latest_data=None):
        ...
        if current_time - self.last_oh_check_time >= 3:
            is_oh, reason, score = oh_judgment_model_ecg_ppg(features)
            
            # 计算风险等级（0-4）
            risk_level = calculate_risk_level(is_oh, reason, score, features)
            self.last_risk_level = risk_level
            self.last_reason = reason
            
            self.oh_alert.emit(is_oh, reason, score)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle('体位性低血压快速筛查系统 v2.4 稳定版 (支持串口输出)')
        ...
        input_control_layout.addWidget(QLabel('输入串口:'))
        input_control_layout.addWidget(self.port_combo)
        input_control_layout.addWidget(QLabel('波特率:'))
        input_control_layout.addWidget(self.baud_combo)
        input_control_layout.addWidget(self.btn_refresh_ports)
        input_control_layout.addWidget(self.btn_connect)
        left_layout.addLayout(input_control_layout)

        # 输出串口控制
        output_control_layout = QHBoxLayout()
        self.output_port_combo = QComboBox()
        self.output_port_combo.setMinimumWidth(220)
        self.output_port_combo.addItem('禁用输出', '')
        self.output_baud_combo = QComboBox()
        self.output_baud_combo.addItems(['9600', '115200', '921600'])
        self.output_baud_combo.setCurrentText(str(OUTPUT_BAUDRATE))
        self.btn_refresh_output_ports = QPushButton('刷新')
        self.btn_refresh_output_ports.clicked.connect(self.refresh_output_serial_ports)
        self.btn_connect_output = QPushButton('连接输出')
        self.btn_connect_output.clicked.connect(self.toggle_output_connection)
        self.output_status_label = QLabel('状态: 未连接')
        
        output_control_layout.addWidget(QLabel('输出串口:'))
        output_control_layout.addWidget(self.output_port_combo)
        output_control_layout.addWidget(QLabel('波特率:'))
        output_control_layout.addWidget(self.output_baud_combo)
        output_control_layout.addWidget(self.btn_refresh_output_ports)
        output_control_layout.addWidget(self.btn_connect_output)
        output_control_layout.addWidget(self.output_status_label)
        ...
        bottom_control_layout.addWidget(self.btn_start)
        bottom_control_layout.addWidget(self.btn_save)
        bottom_control_layout.addWidget(self.btn_export)
        ...
        param_layout.addWidget(sqi_label, 4, 0)
        param_layout.addWidget(self.sqi_bar, 4, 1, 1, 2)
        ...
        self.risk_level_label = QLabel('未检测')
        self.risk_level_label.setAlignment(Qt.AlignCenter)
        self.risk_level_label.setStyleSheet('font-size: 24px; font-weight: bold; color: gray; padding: 10px;')
        risk_layout.addWidget(self.risk_level_label)
        
        # 风险等级说明
        risk_info = QLabel('0:数据不足 | 1:正常 | 2:轻度 | 3:中度 | 4:重度')
        risk_info.setStyleSheet('color: gray; font-size: 11px;')
        risk_info.setAlignment(Qt.AlignCenter)
        ...
        self.alert_text.setMaximumHeight(80)
        ...
        self.event_table.setHorizontalHeaderLabels(['时间', '风险等级', '原因', '状态', '已发送'])
        ...
        self.refresh_output_serial_ports()
    ...
    def refresh_output_serial_ports(self):
        current_port = self.output_port_combo.currentData() if hasattr(self, 'output_port_combo') else ''
        self.output_port_combo.clear()
        self.output_port_combo.addItem('禁用输出', '')
        try:
            ports = list(serial.tools.list_ports.comports())
        except Exception as e:
            self.append_debug(f'输出串口读取失败: {e}')
            return
        for port in ports:
            display_name = f'{port.device} - {port.description}'
            self.output_port_combo.addItem(display_name, port.device)
        # 恢复之前的选择
        for i in range(self.output_port_combo.count()):
            if self.output_port_combo.itemData(i) == current_port:
                self.output_port_combo.setCurrentIndex(i)
                break
    ...
    def toggle_connection(self):
        ...
        self.receiver = SerialTextReceiver(port=selected_port, baudrate=baud)
    ...
    def toggle_output_connection(self):
        """切换输出串口连接"""
        if self.output_sender and self.output_sender.isRunning():
            self.output_sender.disconnect()
            self.output_sender.quit()
            self.output_sender.wait(1000)
            self.btn_connect_output.setText('连接输出')
            self.output_port_combo.setEnabled(True)
            self.output_baud_combo.setEnabled(True)
            self.output_status_label.setText('状态: 未连接')
            self.append_debug('输出串口已断开')
            return

        selected_port = self.output_port_combo.currentData()
        if not selected_port:
            QMessageBox.information(self, '提示', '请选择输出串口或选择"禁用输出"')
            return

        baud = int(self.output_baud_combo.currentText())
        self.output_sender = SerialOutputSender(port=selected_port, baudrate=baud)
        self.output_sender.connection_status.connect(self.on_output_connection_status)
        self.output_sender.debug_message.connect(self.append_debug)
        self.output_sender.send_success.connect(self.on_output_sent)

        if self.output_sender.connect():
            self.output_sender.start()
            self.btn_connect_output.setText('断开输出')
            self.output_port_combo.setEnabled(False)
            self.output_baud_combo.setEnabled(False)
            self.output_status_label.setText('状态: 已连接')
            self.append_debug(f'输出串口连接成功: {selected_port} @ {baud}')
        else:
            QMessageBox.warning(self, '连接失败', f'无法连接到输出串口: {selected_port}')
    ...
    def on_output_sent(self, risk_level, reason):
        """输出发送成功后的回调"""
        risk_names = ['数据不足', '正常', '轻度', '中度', '重度']
        self.append_debug(f'✓ 已发送风险等级到设备: {risk_level} - {risk_names[risk_level]}')
    ...
    def on_oh_alert(self, is_oh, reason, score):
        if self.processor:
            # 获取风险等级（0-4）
            risk_level = self.processor.last_risk_level
            
            # 更新风险等级显示
            risk_names = ['数据不足', '正常', '轻度低血压', '中度低血压', '重度低血压']
            risk_colors = ['gray', 'green', 'orange', 'red', 'darkred']
            self.risk_level_label.setText(f'风险等级: {risk_level} - {risk_names[risk_level]}')
            self.risk_level_label.setStyleSheet(f'font-size: 24px; font-weight: bold; color: {risk_colors[risk_level]}; padding: 10px;')
            
            # 发送到输出串口（如果已连接）
            if self.output_sender and self.output_sender.enabled:
                self.output_sender.update_risk_level(risk_level, reason)
        ...
        if is_oh:
            ...
            self.event_table.setItem(row, 1, QTableWidgetItem(str(self.processor.last_risk_level)))
            self.event_table.setItem(row, 2, QTableWidgetItem(reason[:40]))
            self.event_table.setItem(row, 3, QTableWidgetItem('未处理'))
            self.event_table.setItem(row, 4, QTableWidgetItem('否'))
            ...
            if self.patient_info:
                try:
                    cursor = self.conn.cursor()
                    cursor.execute(
                        'INSERT INTO events (patient_id, event_time, risk_score, risk_level, reason, sent_to_device) VALUES (?, ?, ?, ?, ?, ?)',
                        (self.patient_info.get('id', ''), datetime.now(), score, self.processor.last_risk_level, reason, 0)
                    )
                    self.conn.commit()
                except Exception as e:
    ...
    def export_report(self):
        ...
        if filename:
            ...
            if df.empty:
                df = pd.DataFrame(columns=['time', 'risk_score', 'risk_level', 'reason', 'status', 'sent'])
            df.to_csv(filename, index=False)
