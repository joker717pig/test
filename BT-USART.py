...
BP_BUFFER_SIZE = 1000  # 血压/呼吸波形缓冲区
...
def detect_bp_peaks(bp_signal, fs):
    """检测血压信号的脉搏波峰值（使用呼吸波数据）"""
    if len(bp_signal) < fs * 2:  # 至少2秒数据
        return np.array([]), np.array([]), bp_signal

    # 带通滤波 (0.5-8 Hz)
    bp_filtered = bandpass_filter(bp_signal, 0.5, 8, fs)

    # 寻找收缩压峰值
    min_distance = int(0.4 * fs)
    height_threshold = np.percentile(bp_filtered, 70)
    ...
    return systolic_peaks, [], bp_filtered


def calculate_ptt_ecg_bp(ecg_signal, bp_signal, fs):
    """计算ECG R波到血压脉搏波的PTT"""
    # 检测R波
    r_peaks, ecg_filtered, _ = detect_r_peaks_ecg(ecg_signal, fs)

    # 检测血压波
    bp_peaks, _, bp_filtered = detect_bp_peaks(bp_signal, fs)
...
def calculate_bp_features(bp_signal, peaks, fs):
    """计算血压波形特征"""
    if len(peaks) < 3:
        return None

    amplitudes = bp_signal[peaks]
    ...
    for i in range(len(peaks) - 1):
        peak1 = peaks[i]
        peak2 = peaks[i + 1]
        segment = bp_signal[peak1:peak2]
        if len(segment) > 0:
            diastolic = np.min(segment)
            pulse_pressure = bp_signal[peak1] - diastolic
    ...
    for peak in peaks:
        ...
        segment = bp_signal[start_idx:peak]
        if len(segment) > 0:
...
class SerialOHDetector:
    def __init__(self, port, baud_rate, fs=100):
        ...
        self.bp_buffer = deque(maxlen=BP_BUFFER_SIZE)  # 呼吸/血压波形（通道1）
    ...
    def parse_serial_data(self, line):
        ...
        try:
            ...
            if "HR:" in line and "SPO2:" in line and "B:" in line and "A:" in line:
                ...
                self.bp_buffer.append(bp_value)
    ...
    def extract_features(self):
        """从缓冲区提取特征"""
        if len(self.ecg_buffer) < FS * 3 or len(self.bp_buffer) < FS * 3:
            return None  # 数据不足

        # 转换为numpy数组
        ecg_signal = np.array(self.ecg_buffer)
        bp_signal = np.array(self.bp_buffer)

        features = {}

        # 1. 计算PTT
        ptt_info, has_ptt = calculate_ptt_ecg_bp(ecg_signal, bp_signal, self.fs)
        ...
        # 3. 提取血压特征
        bp_peaks, _, _ = detect_bp_peaks(bp_signal, self.fs)
        bp_features = calculate_bp_features(bp_signal, bp_peaks, self.fs)
    ...
    def print_status(self):
        ...
        print(f"  呼吸波形: {len(self.bp_buffer)}/{BP_BUFFER_SIZE}")
