...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle('体位性低血压快速筛查系统 v2.3 稳定版 (文本串口兼容-PPG)')
        ...
        self.event_table.setHorizontalHeaderLabels(['时间', '风险评分', '原因', '状态'])
    ...
    def on_oh_alert(self, is_oh, reason, score):
        if is_oh:
            ...
            self.event_table.setItem(row, 1, QTableWidgetItem(str(score)))
            self.event_table.setItem(row, 2, QTableWidgetItem(reason[:40]))
            self.event_table.setItem(row, 3, QTableWidgetItem('未处理'))
            self.event_records.append({'time': datetime.now().isoformat(timespec='seconds'), 'risk_score': score, 'reason': reason, 'status': '未处理'})
            if self.patient_info:
                try:
                    cursor = self.conn.cursor()
                    cursor.execute(
                        'INSERT INTO events (patient_id, event_time, risk_score, reason) VALUES (?, ?, ?, ?)',
                        (self.patient_info.get('id', ''), datetime.now(), score, reason)
                    )
                    self.conn.commit()
                except Exception as e:
