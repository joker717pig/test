...
class SerialSender(QThread):
    ...
    def connect(self, port):
        ...
        try:
            ...
            self.running = True
            self.send_status.emit(f'发送串口已连接: {self.port}')
            return True
        except Exception as e:
            self.send_status.emit(f'发送串口连接失败: {e}')
            return False
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                self.send_status.emit(f'发送错误: {e}')
                time.sleep(0.1)
...
class RealTimeProcessor(QObject):
    ...
    def __init__(self, fs=FS):
        ...
        self.serial_sender = None  # 新增：串口发送器引用
        self.last_sent_risk_level = -1  # 新增：上次发送的风险等级

    def set_serial_sender(self, sender):
        """设置串口发送器"""
    ...
    def process_features(self, latest_data=None):
        ...
        if current_time - self.last_oh_check_time >= 3:
            ...
            # 通过串口发送风险等级（如果有变化）
            if self.serial_sender and risk_level != self.last_sent_risk_level:
                self.serial_sender.send_risk_level(risk_level)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.processor = None
        self.receiver = None
        self.serial_sender = None  # 新增：串口发送器
        self.patient_info = {}
        self.event_records = []
    ...
    def init_ui(self):
        self.setWindowTitle('体位性低血压快速筛查系统 v2.3 稳定版 + 串口输出 (文本串口兼容-PPG)')
        self.setGeometry(100, 100, 1450, 900)  # 稍微调高一点以容纳新的控件
        ...
        control_layout.addWidget(QLabel('接收串口:'))
        ...
        self.send_port_combo.setMinimumWidth(150)
        self.send_baud_combo = QComboBox()
        self.send_baud_combo.addItems(['115200', '921600'])
        self.send_baud_combo.setCurrentText(str(BAUDRATE))
        self.btn_refresh_send_ports = QPushButton('刷新发送串口')
        self.btn_refresh_send_ports.clicked.connect(self.refresh_send_serial_ports)
        self.btn_connect_send = QPushButton('连接发送串口')
        self.btn_connect_send.clicked.connect(self.toggle_send_connection)
        self.send_status_label = QLabel('发送状态: 未连接')
        
        send_control_layout.addWidget(QLabel('发送串口:'))
        send_control_layout.addWidget(self.send_port_combo)
        send_control_layout.addWidget(QLabel('波特率:'))
        send_control_layout.addWidget(self.send_baud_combo)
        send_control_layout.addWidget(self.btn_refresh_send_ports)
        send_control_layout.addWidget(self.btn_connect_send)
        send_control_layout.addWidget(self.send_status_label)
        send_control_layout.addStretch()
        
        left_layout.addLayout(send_control_layout)
        ...
        self.risk_level_label = QLabel('0 - 数据不足')
        self.risk_level_label.setStyleSheet('font-weight: bold; color: red; font-size: 14px;')
        ...
        self.refresh_send_serial_ports()
    ...
    def refresh_send_serial_ports(self):
        """刷新发送串口列表"""
        current_port = self.send_port_combo.currentData() if hasattr(self, 'send_port_combo') else None
        self.send_port_combo.clear()
        try:
            ports = list(serial.tools.list_ports.comports())
        except Exception as e:
            self.send_port_combo.addItem('串口读取失败', '')
            self.btn_connect_send.setEnabled(False)
            return
        if not ports:
            self.send_port_combo.addItem('未发现串口', '')
            self.btn_connect_send.setEnabled(False)
            return
        restore_index = 0
        for i, port in enumerate(ports):
            display_name = f'{port.device} - {port.description}'
            self.send_port_combo.addItem(display_name, port.device)
            if current_port and port.device == current_port:
                restore_index = i
        self.send_port_combo.setCurrentIndex(restore_index)
        self.btn_connect_send.setEnabled(True)
    ...
    def toggle_connection(self):
        ...
        # 如果发送串口已连接，将发送器设置给处理器
        if self.serial_sender and self.serial_sender.isRunning():
            self.processor.set_serial_sender(self.serial_sender)
        ...
        self.processor.risk_level_ready.connect(self.on_risk_level_ready)  # 新增
    ...
    def toggle_send_connection(self):
        """切换发送串口连接"""
        if self.serial_sender and self.serial_sender.isRunning():
            self.serial_sender.disconnect()
            self.serial_sender.quit()
            self.serial_sender.wait(1000)
            self.btn_connect_send.setText('连接发送串口')
            self.send_port_combo.setEnabled(True)
            self.send_baud_combo.setEnabled(True)
            self.btn_refresh_send_ports.setEnabled(True)
            self.send_status_label.setText('发送状态: 未连接')
            if self.processor:
                self.processor.set_serial_sender(None)
            return

        self.refresh_send_serial_ports()
        selected_port = self.send_port_combo.currentData()
        if not selected_port:
            QMessageBox.warning(self, '提示', '请先选择发送串口')
            return

        baud = int(self.send_baud_combo.currentText())
        self.serial_sender = SerialSender(port=selected_port, baudrate=baud)
        self.serial_sender.send_status.connect(self.append_send_status)
        
        if self.serial_sender.connect(selected_port):
            self.serial_sender.start()
            self.btn_connect_send.setText('断开发送串口')
            self.send_port_combo.setEnabled(False)
            self.send_baud_combo.setEnabled(False)
            self.btn_refresh_send_ports.setEnabled(False)
            self.send_status_label.setText(f'发送状态: 已连接 {selected_port}')
            
            # 如果处理器已存在，设置发送器
            if self.processor:
                self.processor.set_serial_sender(self.serial_sender)
        else:
            QMessageBox.warning(self, '连接失败', f'无法连接到发送串口设备: {selected_port}')
    ...
    def export_report(self):
        ...
        if filename:
            ...
            if df.empty:
                df = pd.DataFrame(columns=['time', 'risk_score', 'reason', 'status'])
            df.to_csv(filename, index=False)
