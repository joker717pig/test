...
6. 【新增】支持在同一串口上接收数据并发送OH判定结果（双向通信）
"""
...
class SerialDualPort(QThread):
    """
    双向串口通信线程
    在同一个串口上接收设备数据，并发送OH判定结果
    """
    ...
    def connect(self):
        try:
            ...
            self.debug_message.emit(f'串口已连接: {self.port} @ {self.baudrate} (双向通信)')
            return True
        ...
        while self.running:
            try:
                if self.serial and self.serial.is_open:
                    ...
                    # 检查是否有新数据且到了发送时间
                    QMutexLocker(self.lock)
                    if self.new_output_available and (current_time - self.last_send_time >= OUTPUT_INTERVAL):
                        # 发送格式: "RISK:X\n" 其中X为0-4
                        ...
                        try:
                            ...
                            self.debug_message.emit(f'发送失败: {e}')
                ...
                self.debug_message.emit(f'通信错误: {e}')
                time.sleep(0.1)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle('体位性低血压快速筛查系统 v2.5 单串口双向通信版')
    ...
    def toggle_connection(self):
        if self.serial_comm and self.serial_comm.isRunning():
            self.serial_comm.disconnect()
            self.serial_comm.quit()
            self.serial_comm.wait(1000)
        ...
        self.serial_comm = SerialDualPort(port=selected_port, baudrate=baud)
        self.processor = RealTimeProcessor()
        self.serial_comm.data_received.connect(self.processor.on_data_received)
        self.serial_comm.connection_status.connect(self.on_connection_status)
        self.serial_comm.debug_message.connect(self.append_debug)
        self.serial_comm.send_success.connect(self.on_output_sent)
        ...
        if self.serial_comm.connect():
            ...
            self.statusBar().showMessage(f'设备连接成功: {selected_port} @ {baud} (双向通信)')
        else:
    ...
    def on_oh_alert(self, is_oh, reason, score):
        if self.processor:
            ...
            # 发送到串口
            if self.serial_comm:
                self.serial_comm.update_risk_level(risk_level, reason)
        ...
        if is_oh:
            ...
            self.event_table.setItem(row, 4, QTableWidgetItem('待发送'))
